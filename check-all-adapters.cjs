...
async function main() {
  ...
  var companies = await sql`SELECT DISTINCT c.id, c.name, a.adapter_type FROM companies c JOIN agents a ON c.id = a.company_id WHERE a.role = 'ceo' ORDER BY c.created_at DESC LIMIT 10`;
