...
const permissionLabels: Record<PermissionKey, string> = {
  "agents:create": t("companyAccess.createAgents"),
  "users:invite": t("companyAccess.inviteHumansAndAgents"),
  "users:manage_permissions": t("companyAccess.manageMembers"),
  "tasks:assign": t("companyAccess.assignTasks"),
  "tasks:assign_scope": t("companyAccess.assignScopedTasks"),
  "tasks:manage_active_checkouts": t("companyAccess.manageActiveTaskCheckouts"),
  "joins:approve": t("companyAccess.approveJoins"),
  "environments:manage": t("companyAccess.manageEnvironments"),
};

function formatGrantSummary(member: CompanyMember) {
  if (member.grants.length === 0) return t("companyAccess.noExplicitGrants");
...
export function CompanyAccess() {
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.failedToApprove"),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.failedToReject"),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
      ...
      pushToast({
        ...
        body:
          result.reassignedIssueCount > 0
            ? t("companyAccess.assignedIssuesCleaned", { count: result.reassignedIssueCount })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.failedToRemove"),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError"),
        tone: "error",
      });
    },
    ...
    <div className="max-w-6xl space-y-8">
      ...
      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {t("companyAccess.instanceAdminNote")}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{t("companyAccess.manageMembership")}</p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("companyAccess.pendingHumanJoins")}</h3>
                <p className="text-sm text-muted-foreground">{t("companyAccess.pendingHumanJoinsDesc")}</p>
              </div>
              <Badge variant="outline">{pendingHumanJoinRequests.length} {t("companyAccess.pending")}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("companyAccess.unknownHumanRequester")
                  }
                  subtitle={
                  ...
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes} ${t("companyAccess.humanJoinRequests")}${request.invite.humanRole ? ` • ${t("companyAccess.defaultRole", { role: request.invite.humanRole })}` : ""}`
                      : t("companyAccess.inviteMetadataUnavailable")
                  }
                  detail={t("companyAccess.submittedAt", { date: new Date(request.createdAt).toLocaleString() })}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_minmax(0,1.2fr)_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("companyAccess.account")}</div>
            <div>{t("companyAccess.role")}</div>
            <div>{t("companyAccess.status")}</div>
            <div>{t("companyAccess.grants")}</div>
            <div className="text-right">{t("companyAccess.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.noMembersYet")}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companyAccess.editMemberHint")}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">{t("companyAccess.grantsTab")}</h3>
                  <p className="text-sm text-muted-foreground">{t("companyAccess.grantsDescription")}</p>
                </div>
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">{t("companyAccess.implicitGrantsFromRole")}</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? t("companyAccess.implicitFromRoleHint", { role: HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] })
                      : t("companyAccess.noRoleSelected")}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("companyAccess.includedImplicitly", { role: draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : t("companyAccess.unset") })}
                          </span>
                        ) : null}
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("companyAccess.storedExplicitlyFor")}
                          </span>
                        ) : null}
                      </span>
                    </label>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? t("companyAccess.saving") : t("companyAccess.saveAccess")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companyAccess.removeMemberHint", { member: memberDisplayName(removingMember) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("companyAccess.checkingIssues")
                    : t("companyAccess.openAssignedIssues", { count: assignedIssues.length })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("companyAccess.issueReassignment")}</div>
                  <select
...
function memberDisplayName(member: CompanyMember | null) {
  if (!member) return t("companyAccess.memberDisplayName");
