  ...
  try {
    ...
    const company = await page.evaluate(async () => {
      const res = await fetch('/api/companies', {
        ...
        credentials: 'include',
        body: JSON.stringify({ name: 'TestFix' })
      });
      return res.json();
    });
    ...
    // Parse hire response correctly
    const approvalId = hire.approval ? hire.approval.id : hire.approvalId;
    const ceoId = hire.agentId || (hire.approval ? hire.approval.agentId : null);
    LOG('Hire: approvalId=' + approvalId + ' ceoId=' + ceoId);

    if (approvalId && ceoId) {
      // Approve
      LOG('Approving...');
      await page.evaluate(async (aid) => {
        await fetch('/api/approvals/' + aid + '/approve', { method: 'POST', credentials: 'include' });
      }, approvalId);
      await s(3000);
      LOG('Approved!');
      ...
      LOG('Issue: id=' + issue.id + ' garbled=' + garbled);
      ...
      LOG('Hire incomplete: ' + JSON.stringify(hire));
    }
